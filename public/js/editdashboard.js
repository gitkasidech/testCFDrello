var countlist = 0;
// alert($("#boardid").val())
// alert($("#selectcomplete").val())
// alert($("#selectinprogress").val())
// alert($("#selectbacklog").val())
var board_id = $("#boardid").val();
var idComplete = $("#idComplete").val()
var idInprogress = $("#idInprogress").val()
var idBacklog = $("#idBacklog").val()
var colorComplete = $("#colorComplete").val()
var colorInprogress = $("#colorInprogress").val()
var colorBacklog = $("#colorBacklog").val()
                ...
                if (board_id == board.id) {
                    ...
                    Trello.get('/boards/' + board.id + '/lists', function(lists) {
                        $.each(lists, function(index, list) {
                            ...
                            if (idComplete == list.id) {
                                $("#complete" + countlist).prop("checked", true);
                                $('#listscomplete')
                                    .append($("<option value=" + list.id + " color='#000000'></option>")
                                        .text(list.name)
                                    )
                            }
                            ...
                            if (idInprogress == list.id) {
                                $("#inprogress" + countlist).prop("checked", true);
                                $('#listsinprogress')
                                    .append($("<option value=" + list.id + " color='#000000'></option>")
                                        .text(list.name)
                                    )
                            ...
                            if (idBacklog == list.id) {
                                $("#backlog" + countlist).prop("checked", true);
                                $('#listsbacklog')
                                    .append($("<option value=" + list.id + " color='#000000'></option>")
                                        .text(list.name)
                                    )
...
function selectInprogressChange() {}
